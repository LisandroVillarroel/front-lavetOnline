...
@Component({
    ...
    styleUrls: ['./elimina-especie.component.scss'],
...
export class EliminaEspecieComponent implements OnInit {
  ...
  readonly dialogRef = inject(MatDialogRef<EliminaEspecieComponent>);
  ...
  enviar() {
    ...
    this.especieService.deleteDataEspecie(this.data).subscribe({
      next: (dato) => {
        this.spinnerService.esconder();
        if (dato.codigo === 200) {

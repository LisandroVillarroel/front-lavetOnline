...
@Component({
    ...
    styleUrls: ['./elimina-raza.component.scss'],
...
export class EliminaRazaComponent implements OnInit {
  ...
  readonly dialogRef = inject(MatDialogRef<EliminaRazaComponent>);
  readonly data = inject<IRaza>(MAT_DIALOG_DATA);

  public razaService = inject(RazaService);
  ...
  enviar() {
    ...
    this.razaService.deleteDataRaza(this.data).subscribe({
      next: (dato) => {
        this.spinnerService.esconder();
        if (dato.codigo === 200) {
